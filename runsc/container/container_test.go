...
func testCheckpointRestore(t *testing.T, conf *config.Config, compression statefile.CompressionLevel, newSpecWithScript func(string) *specs.Spec) {
	...
	if !cont2.Sandbox.IsRestored() {
		t.Fatalf("sandbox returned wrong value for IsRestored(), got: false, want: true")
	}

	if cont2.Sandbox.IsCheckpointed() {
		t.Fatalf("sandbox returned wrong value for IsCheckpointed(), got: true, want: false")
	}
...
func TestCheckpointResume(t *testing.T) {
	for name, conf := range configs(t, true /* noOverlay */) {
		t.Run(name, func(t *testing.T) {
			...
			if !cont.Sandbox.IsCheckpointed() {
				t.Fatalf("sandbox returned wrong value for IsCheckpointed(), got: false, want: true")
			}

			if cont.Sandbox.IsRestored() {
				t.Fatalf("sandbox returned wrong value for IsRestored(), got: true, want: false")
			}
			cont.Destroy()
		})
	}
